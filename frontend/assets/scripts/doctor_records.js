  ...
  data.forEach(record => {
    html += `
      <tr>
        ...
        <td><textarea id="doctor-notes-${record.record_id}" class="form-control">${record.doctor_notes || ""}</textarea></td>
        <td><textarea class="form-control" readonly>${record.patient_notes || ""}</textarea></td>

        <td>
          <select class="form-select" onchange="updateStatusTag(${record.record_id}, this.value)">
            <option ${(!record.status_tag || record.status_tag === "New") ? "selected" : ""}>New</option>
            <option ${record.status_tag === "Follow-up" ? "selected" : ""}>Follow-up</option>
            <option ${record.status_tag === "Resolved" ? "selected" : ""}>Resolved</option>
          </select>
        </td>

        <td>
          ${record.attachment_url ? `<a href="/${record.attachment_url}" target="_blank">View</a>` : ""}
          <input type="file" onchange="uploadFile(${record.record_id}, this)">
        </td>

        <td>
          <button class="btn btn-primary btn-sm" onclick="updateDoctorNotes(${record.record_id})">Save Notes</button>
          <button class="btn btn-secondary btn-sm" onclick="viewHistory(${record.record_id})">History</button>
        </td>
      </tr>`;
  });
...
async function uploadFile(recordId, input) {
  ...
  fetchDoctorPatientRecords(); // refresh table
}
