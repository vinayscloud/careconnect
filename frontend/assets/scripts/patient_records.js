    ...
    data.forEach(record => {
      html += `
        <tr>
          ...
          <td><textarea class="form-control" readonly>${record.doctor_notes || ""}</textarea></td>
          <td><textarea class="form-control" id="patient-note-${record.record_id}">${record.patient_notes || ""}</textarea></td>
  
          <td><span class="badge bg-info">${record.status_tag || "New"}</span></td>
          ...
          <td>
            <button class="btn btn-primary btn-sm" onclick="savePatientNote(${record.record_id})">Save Note</button>
